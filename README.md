...
src/
  app/
    ...
    account/forgot-password     استعادة كلمة المرور عبر رمز واتساب
    admin/                      لوحة الإدارة (لوحة معلومات، طلبات، كتالوج، محادثات واتساب)
    about/, contact/            صفحتا "من نحن" و"تواصل معنا"
    icon.tsx, apple-icon.tsx    أيقونة الموقع (تُولَّد بدل favicon.ico ثابت)
    opengraph-image.tsx         صورة مشاركة الموقع على وسائل التواصل
    product/[slug]/opengraph-image.tsx  صورة مشاركة مخصّصة لكل خدمة
    api/orders/                 إنشاء الطلبات + رفع إثبات الدفع + إنشاء/تسجيل دخول التاجر
    api/account/                تسجيل دخول/خروج التاجر، استعادة كلمة المرور
    api/admin/                  تسجيل الدخول، تحديث حالة الطلبات/الكتالوج، إرسال واتساب، تصدير CSV
  ...
  assets/cairo-bold.ttf         خط لتوليد صور OG (لا علاقة له بخط الموقع نفسه، next/font يتكفل به)
  proxy.ts                      حماية /admin, /api/admin, /account, /api/account (Next.js Proxy)
prisma/
  schema.prisma                 نماذج Product, Merchant, Order, WhatsappMessage, PasswordResetCode
  seed.ts                       بيانات ابتدائية لـ 8 خدمات أتمتة
vitest.global-setup.ts          يبني قاعدة بيانات اختبار معزولة من ملفات الترحيل الحقيقية
```
...
`npm run test` (Vitest) يشغّل نوعين من الاختبارات:

- **اختبارات وحدة** للمنطق الأساسي الذي لا يحتاج قاعدة بيانات: تشفير كلمات المرور، تحديد معدل
  الطلبات، التحقق من صحة بيانات الخدمة، بناء فلتر الطلبات، اقتطاع نصوص صور المشاركة، وتوليد
  HTML الفاتورة (بما في ذلك التأكد من عدم إمكانية حقن HTML عبر اسم التاجر أو المتجر).
- **اختبارات تكامل** تستدعي مسارات الـ API الحقيقية مباشرة (`POST /api/orders`،
  `POST /api/admin/login`، `GET /api/invoices/[orderNumber]`) على قاعدة بيانات SQLite مؤقتة
  ومعزولة تُبنى تلقائيًا قبل كل تشغيل (`vitest.global-setup.ts` يعيد تشغيل ملفات الترحيل
  الحقيقية من `prisma/migrations/` على ملف مؤقت، بدل نسخة يدوية من المخطط قد تفقد التزامن).
  هذا يغطي: إنشاء الطلبات والتحقق من صحتها، إنشاء/تسجيل دخول حساب التاجر تلقائيًا، رفض كلمة
  مرور خاطئة عند تكرار الطلب بنفس الرقم، تحديد معدل الدخول لحساب الإدارة، وحماية الوصول
  للفاتورة (المالك أو الإدارة فقط). لا تزال مكوّنات الواجهة (React) غير مغطاة.

لا تحتاج قاعدة بيانات التطوير (`prisma/dev.db`) لتشغيل الاختبارات — تُستخدم قاعدة بيانات
منفصلة تُحذف تلقائيًا بعد كل تشغيل، فلا خطر من تلويث بيانات حقيقية أثناء الاختبار.
